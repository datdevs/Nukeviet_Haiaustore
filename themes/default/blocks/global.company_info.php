...
if (!nv_function_exists('nv_company_info')) {
    ...
    function nv_company_info_config($module, $data_block, $lang_block)
    {
        ...
        $html .= '</div>';
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">Khoảng giá:</label>';
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_company_responsibility" value="' . $data_block['company_responsibility'] . '"></div>';
        $html .= '</div>';
        // $html .= '<div class="form-group">';
    ...
    function nv_company_info($block_config)
    {
        ...
        $xtpl->assign('SITE_BANNER', NV_REAL_DOMAIN . $global_config['site_banner']);
        $xtpl->assign('SITE_LOGO', NV_REAL_DOMAIN . $global_config['site_logo']);
        $xtpl->assign('DATA', $block_config);

        $xtpl->assign('SCHEMA_NAME', !empty($block_config['company_name']) ? $block_config['company_name'] : $global_config['site_name']);
        $xtpl->assign('SCHEMA_PHONE', $global_config['site_phone']);
        $xtpl->assign('SCHEMA_EMAIL', $global_config['site_email']);
        $xtpl->assign('SCHEMA_ADDRESS', $block_config['company_address']);
        $xtpl->assign('SCHEMA_PRICE_RANGE', $block_config['company_responsibility']);
