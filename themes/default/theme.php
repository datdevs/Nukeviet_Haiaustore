...
function nv_site_theme($contents, $full = true)
{
    ...
    $xtpl->assign('NV_REAL_DOMAIN', NV_REAL_DOMAIN);
    ...
    $html_js[] = [
        'ext' => 1,
        'content' => NV_REAL_DOMAIN . 'themes/' . $global_config['module_theme'] . '/js/main.js'
    ];
    $html_js[] = [
        'ext' => 1,
    ...
    $xtpl->assign('THEME_SITE_HREF', NV_MY_DOMAIN . nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA, true));
    $xtpl->assign('LOGO_SRC', NV_REAL_DOMAIN . $global_config['site_logo']);

    if (empty($global_config['site_banner'])) {
        $xtpl->assign('BANNER_SRC', NV_REAL_DOMAIN . 'themes/' . $global_config['module_theme'] . '/images/header.png');
    } else {
        $xtpl->assign('BANNER_SRC', NV_REAL_DOMAIN . $global_config['site_banner']);
    }
    ...
    if ($full) {
        ...
        if (!$home) {
            ...
            if (!empty($array_mod_title_copy)) {
                $border = 2;
                $count_title = count($array_mod_title_copy) + 1;
                foreach ($array_mod_title_copy as $arr_cat_title_i) {
                    $arr_cat_title_i['position'] = $border++;
                    $arr_cat_title_i['active'] = $arr_cat_title_i['position'] == $count_title ? 'active' : null;
