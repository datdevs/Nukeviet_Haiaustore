...
class Optimizer
{
    ...
    /**
     * imagesCallback()
     * Optimized lazyload images by Vujidev
     *
     * @param array $matches
     * @return string
     */
    private function imagesCallback($matches)
    {
        if (preg_match("/(.*class=['\"](.*)non-lazy(.*)['\"].*)/i", $matches[0]) or preg_match("/(.*class=['\"](.*)lazyload(.*)['\"].*)/i", $matches[0])) {
            return $matches[0];
        } elseif (preg_match('/(class)=("[^"]*")/i', $matches[0])) {
            $matches[0] = str_replace("class=\"", "class=\"lazyload ", $matches[0]);
        } else {
            $matches[0] = str_replace("<img", "<img class=\"lazyload\"", $matches[0]);
        }
        $matches[0] = str_replace("src=", "loading=\"lazy\" data-src=", $matches[0]);
