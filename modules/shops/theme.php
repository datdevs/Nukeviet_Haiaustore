...
function nv_template_view_home($array_data, $compare_id, $pages = '', $sort = 0, $viewtype = 'viewgrid')
{
    ...
    if ($pro_config['home_data'] == 'all') {
        if (function_exists('nv_template_' . $viewtype)) {
            $xtpl->assign('CONTENT', call_user_func('nv_template_' . $viewtype, $array_data, $pages, $sort, $viewtype));
        }
    ...
    if (!empty($data_content)) {
        ...
        if ($num_images == 1) {
            ...
            foreach ($data_content['image'] as $image) {
                ...
                if ($stt == 1) {
                    $xtpl->parse('main.image.loop.active');
                }
        ...
        if (!$popup) {
            ...
            if (!empty($data_others)) {
                $html = call_user_func('nv_template_viewgrid', $data_others, '', 0, 'viewgrid', false, 4);
                $xtpl->assign('OTHER', $html);
                $xtpl->parse('main.product_detail.other');
            }
            if (!empty($array_other_view)) {
                $html = call_user_func('nv_template_viewgrid', $array_other_view, '', 0, 'viewgrid', false, 4);
                    ...
                    foreach ($subid as $sub_gr_id) {
                        ...
                        if ($sub_info['in_order']) {
                            ...
                            if (!empty($sub_info['image'])) {
                                $xtpl->assign('IMG', NV_REAL_DOMAIN . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $sub_info['image']);
                                if (sizeof($subid) == 1) {
                                    $xtpl->parse('main.group.items.loop.image.active');
                                    $xtpl->parse('main.group.items.loop.image.checked');
                                }
                                $xtpl->parse('main.group.items.loop.image');
                            } else {
                                if (sizeof($subid) == 1) {
                                    $xtpl->parse('main.group.items.loop.checkbox.active');
                                    $xtpl->parse('main.group.items.loop.checkbox.checked');
                                }
                                $xtpl->parse('main.group.items.loop.checkbox');
                            }
...
function uers_order($data_content, $data_order, $total_coupons, $order_info)
{
    ...
    $xtpl->assign('price_total_products', nv_number_format($price_total, nv_get_decimals($pro_config['money_unit'])));
    ...
    if ($module_config[$module_name]['use_shipping']) {
        ...
        if (!empty($shipping_data['list_shops'])) {
            ...
            foreach ($shipping_data['list_shops'] as $rows_i) {
                ...
                $rows_i['selected'] = ($data_order['shipping']['ship_shops_id'] == $rows_i['id'] or $i == 0) ? 'selected' : '';
        ...
        $xtpl->assign('IS_SHIPPING_CHECKED', $data_order['order_shipping'] ? 'checked' : '');
...
function payment($data_content, $data_pro, $data_shipping, $payment_supported, $intro_pay, $point)
{
    ...
    foreach ($data_pro as $pdata) {
        ...
        $xtpl->assign('img_pro', $pdata['homeimgthumb']);
 ...
 * @return
 */
function nv_template_viewgrid($array_data, $page = '', $sort = 0, $viewtype = 'viewgrid', $show_display = true, $nuw_row = null)
{
    global $module_info, $lang_module, $lang_global, $module_name, $module_data, $module_file, $module_upload, $pro_config, $op, $compareid, $global_array_shops_cat, $array_displays;

    $xtpl = new XTemplate('viewgrid.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('MODULE_NAME', $module_name);

    if (!empty($pro_config['show_displays']) && $show_display) {
    ...
    if (!empty($array_data)) {
        $i = 1;
        $xtpl->assign('NUM', $nuw_row !== null ? 12 / $nuw_row : 12 / $pro_config['per_row']);
        ...
        foreach ($array_data as $data_row) {
            ...
            $data_row['cat_title'] = $global_array_shops_cat[$data_row['listcatid']]['title'];
            $data_row['cat_link'] = $global_array_shops_cat[$data_row['listcatid']]['link'];
...
{
    global $lang_tmp, $module_name, $module_file, $pro_config, $module_info, $money_config;

    $xtpl = new XTemplate("block.cart.tpl", NV_ROOTDIR . "/themes/" . $module_info['template'] . "/modules/" . $module_file);
    $xtpl->assign('LANG', $lang_tmp);
    $xtpl->assign('TEMPLATE', $module_info['template']);
    $xtpl->assign('LINK_VIEW', NV_MY_DOMAIN . nv_url_rewrite(NV_BASE_SITEURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&amp;" . NV_NAME_VARIABLE . "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=cart", true));
    ...
    if ($pro_config['active_order'] == '1') {
        if (!empty($array_products)) {
            foreach ($array_products as $product) {
                ...
                $xtpl->parse('main.enable.product.loop');
            }
            if ($pro_config['active_price'] == '1') {
                $xtpl->parse('main.enable.product.price');
            }
            $xtpl->parse('main.enable.product');
        } else {
            $xtpl->parse('main.enable.empty');
        }
