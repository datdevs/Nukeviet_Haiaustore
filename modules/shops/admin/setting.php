...
// Group custom
$groups_list = array();
$result = $db->query('SELECT t1.group_id as group_id, t2.title as title, t1.idsite as idsite FROM ' . NV_GROUPS_GLOBALTABLE . ' as t1 INNER JOIN ' . NV_GROUPSDETAIL_GLOBALTABLE . ' as t2 ON t1.group_id=t2.group_id WHERE t1.group_id NOT IN ( 4, 5, 6 ) AND (t1.idsite = ' . $global_config['idsite'] . ' OR (t1.idsite =0 AND t1.siteus = 1)) ORDER BY t1.idsite, t1.weight');
