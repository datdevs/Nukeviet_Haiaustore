...
if ($order_id > 0 and $checkss == md5($order_id . $global_config['sitekey'] . session_id())) {
    ...
    if (!empty($listid)) {
        ...
        foreach ($listid as $_dbkey => $_proid) {
            ...
            t2.' . NV_LANG_DATA . '_title, t1.money_unit, t1.homeimgfile, t1.homeimgthumb
            ...
            $result = $db->query($sql);
            while (list($id, $listcatid, $publtime, $title, $alias, $hometext, $unit, $money_unit, $homeimgfile, $homeimgthumb) = $result->fetch(3)) {
