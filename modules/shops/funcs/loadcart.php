...
if (!empty($_SESSION[$module_data . '_cart'])) {
    foreach ($_SESSION[$module_data . '_cart'] as $pro_id => $info) {
        ...
        $array_product_num[$pro_id] = $info['num'];
...
if ($get_list_product && !empty($array_product_id)) {
    ...
    while ($row = $result->fetch()) {
        if ($row['homeimgthumb'] == 1) {
            //image thumb
            $row['thumb'] = NV_REAL_DOMAIN . NV_FILES_DIR . '/' . $module_upload . '/' . $row['homeimgfile'];
        } elseif ($row['homeimgthumb'] == 2) {
            //image file
            $row['thumb'] = NV_REAL_DOMAIN . NV_UPLOADS_DIR . '/' . $module_upload . '/' . $row['homeimgfile'];
        } elseif ($row['homeimgthumb'] == 3) {
            //image url
            $row['thumb'] = $row['homeimgfile'];
        } else {
            //no image
            $row['thumb'] = NV_REAL_DOMAIN . 'themes/' . $module_info['template'] . '/images/' . $module_file . '/no-image.jpg';
        }
        $row['link'] = NV_MY_DOMAIN . nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $global_array_shops_cat[$row['listcatid']]['alias'] . '/' . $row['alias'] . $global_config['rewrite_exturl'], true);
        $row['num'] = $array_product_num[$row['id']];
