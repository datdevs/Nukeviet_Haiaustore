...
if (!nv_function_exists('nv_global_bxproduct_center')) {
    ...
    function GetGroupID_Block($pro_id, $group_by_parent = 0, $module_data, $global_array_group)
    {
        global $db, $db_config;
    ...
    {
        global $site_mods, $global_config, $module_config, $lang_module, $module_name, $global_array_shops_cat, $global_array_group, $db_config, $my_head, $db, $pro_config, $money_config, $blockID, $nv_Cache;
        ...
        $xtpl->assign('JS_LINK', NV_REAL_DOMAIN . 'themes/' . $block_theme . '/js/' . $module . '.js');
        ...
        $db->sqlreset()
            ->select('t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.homeimgfile, t1.homeimgthumb , t1.homeimgalt, t1.showprice, t1.product_price, t1.product_number')
        ...
        foreach ($list as $row) {
            ...
            {
                $src_img = NV_REAL_DOMAIN . NV_FILES_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $row['homeimgfile'];
            } elseif ($row['homeimgthumb'] == 2) //image file
            {
                $src_img = NV_REAL_DOMAIN . NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $row['homeimgfile'];
            } elseif ($row['homeimgthumb'] == 3) //image url
            {
                $src_img = $row['homeimgfile'];
            } else //no image
            {
                $src_img = NV_REAL_DOMAIN . 'themes/' . $block_theme . '/images/shops/no-image.jpg';
            }
            ...
            $row['cat_title'] = $global_array_shops_cat[$row['listcatid']]['title'];
            $row['cat_link'] = $global_array_shops_cat[$row['listcatid']]['link'];
            ...
            if ($pro_config['active_order'] == '1' and $pro_config['active_order_non_detail'] == '1') {
                if ($row['showprice'] == '1' && !empty($row['product_price'])) {
                    if ($row['product_number'] > 0) {
                        ...
                        $listgroupid = GetGroupID_Block($row['id'], 0, $mod_data, $global_array_group);
                        $group_requie = 0;
