...
function contact_main_theme($array_content, $array_department, $catsName, $base_url, $checkss)
{
    ...
    if (!empty($array_department)) {
        $default_deparment_key = array_search(1, array_column($array_department, 'is_default')) + 1;
        $default_deparment = $array_department[$default_deparment_key];

        if (!empty($default_deparment)) {
            if (empty($alias_url) and $default_deparment['act'] !== 2) {
                $xtpl->assign('DEP', $default_deparment);

                if (!empty($default_deparment['note'])) {
                    $xtpl->parse('main.dep.note');
                }

                if (!empty($default_deparment['address'])) {
                    $xtpl->parse('main.dep.address');
                }

                if (!empty($default_deparment['phone'])) {
                    $nums = array_map('trim', explode('|', nv_unhtmlspecialchars($default_deparment['phone'])));
                    foreach ($nums as $k => $num) {
                        unset($m);
                        if (preg_match("/^(.*)\s*\[([0-9\+\.\,\;\*\#]+)\]$/", $num, $m)) {
                ...
                if (!empty($default_deparment['email'])) {
                    $emails = array_map('trim', explode(',', $default_deparment['email']));
                    foreach ($emails as $k => $email) {
                        $xtpl->assign('EMAIL', $email);
                        if ($k) {
                            $xtpl->parse('main.dep.email.item.comma');
                ...
                if (!empty($default_deparment['others'])) {
                    $others = json_decode($default_deparment['others'], true);

                    if (!empty($others)) {
                        foreach ($others as $key => $value) {
                            if (!empty($value)) {
                                if (strtolower($key) == 'maps') {
                                    $xtpl->assign('MAPSLINK', $value);
                                    $xtpl->parse('main.google_maps');
                                }
                            }
                        }
                    }
