...
$authors = [];
$db->sqlreset()
    ->select('l.alias,l.pseudonym,a.image')
...
while ($row = $result->fetch()) {
    $authors[] = '<a class="blog-entry-meta-link" href="' . NV_MY_DOMAIN . nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=author/' . $row['alias'], true) . '"><div class="blog-entry-author-ava"><img src="https://cartzilla.createx.studio/img/blog/meta/02.jpg" alt="' . $row['pseudonym'] . '"></div>' . $row['pseudonym'] . '</a>';
}
